...
const balancedBrackets3 = function (str) {
  // TODO Data Type Stack을 이용하자!
...
const balancedBrackets4 = function (str) {
  // open 문자와 close 문자를 각 각 배열로 만든다
  // stack 빈 배열을 선언한다.
  let opBracket = ["(", "[", "{"];
  let clBracket = [")", "]", "}"];
  ...
  // 입력으로 받은 str에서 ),],} 중 하나가 나올 떄까지 Stck에 넣는다.
    ...
    if (clBracket.indexOf(nextB) === opBracket.indexOf(openB)) {
      ...
      if (!stack.length) {
        ...
        for (let i = 0; i < remains.length; i++) {
          if (opBracket.includes(remains[i])) stack.push(remains[i]);
...
// ! tests
let output = balancedBrackets4("())()(()");
console.log(output); // --> false

let output2 = balancedBrackets4("[({})]");
console.log(output2); // --> true

let output3 = balancedBrackets4("[(]{)}");
console.log(output3); // --> false

let output4 = balancedBrackets4("(((((((((())))))))))");
